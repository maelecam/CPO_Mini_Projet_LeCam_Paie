...
// Ajout pour cacher toutes les lignes sauf la première
        ...
        panneauGauche.setLayout(new GridLayout(10, 2, 5, 5)); // 10 lignes, 2 colonnes, marges de 5px
        JButton[][] boutonsGauche = new JButton[10][2]; // Tableau pour gérer les boutons gauche

        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 2; j++) {
                boutonsGauche[i][j] = new JButton("G" + (i + 1) + "," + (j + 1));
                panneauGauche.add(boutonsGauche[i][j]); // Ajouter le bouton au panneau gauche
            }
        }

// Panneau droit avec un tableau de 10 lignes et 2 colonnes
        JPanel panneauDroit = new JPanel();
        panneauDroit.setLayout(new GridLayout(10, 2, 5, 5)); // 10 lignes, 2 colonnes, marges de 5px
        JButton[][] boutonsDroit = new JButton[10][2]; // Tableau pour gérer les boutons droit

        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 2; j++) {
                boutonsDroit[i][j] = new JButton("D" + (i + 1) + "," + (j + 1));
                panneauDroit.add(boutonsDroit[i][j]); // Ajouter le bouton au panneau droit
            }
        }

// Ajouter les panneaux gauche et droit au layout principal
        getContentPane().setLayout(new BorderLayout());
        getContentPane().add(panneauGauche, BorderLayout.WEST); // Ajouter le panneau gauche
        getContentPane().add(panneauDroit, BorderLayout.EAST);  // Ajouter le panneau droit
